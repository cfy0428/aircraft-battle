...
p.display.set_caption('飞机大战v.2.0')
...
def store():
	global bullet_timetick,bullet_hit_area,store1,store2,store3,store4,store5,golds,real_bullet_timetick,extra_bullet_num,lives
	if bullet_timetick > 3.5:
	...
	if extra_bullet_num < 10:
		golds_inneed3 = (extra_bullet_num//2+1)**2*5+50
		texti2 = f"需要金币：{golds_inneed3}  提升：{extra_bullet_num}—{extra_bullet_num+2}"
		if store3:
	...
	texth1 = f"回血(G)"
	texth2 = f"需要金币：20"
	textprint(font,600,700,texth1)
	textprint(font,600,730,texth2)
...
def level():
	global levels,level_timetick,level_enemies_now,boss,boss_killed
	level_enemies_all = (levels**1.2)*10
	level_newtime = random.randint(50-3*levels,100-3*levels)
	if level_newtime<30:level_newtime=30
	level_timetick += 1
	if level_enemies_now+levels*3 >= level_enemies_all and enemies == [] :
		level_enemies_now = 0
		level_timetick = 0				
		boss = True
		...
		if level_timetick % level_newtime == 0 and level_enemies_now+levels*3 < level_enemies_all and not boss:
...
class Enemy():
	def __init__(self):
		...
		self.timetick = 0
		if boss:			
			self.img = p.image.load('boss.png')
			self.size = 512
			self.live = round(9*levels**1.2,0)
			self.stepx = 0
			self.stepy = 0.5
			self.type = 4
			self.x = 50
...
def reset_game():
	global timetick,score,lives,playerX,playerY,playerStep,scorea,levels,bullet_timetick,level_timetick,level_timetick,rage_mode,real_bullet_timetick,boss,boss_killed,golds
	timetick = 0
	score = 0
	scorea = 0
	lives = 200
...
class bullet_bye:
	def __init__(self,xpos,ypos):
		...
		self.x = xpos
		self.y = ypos
		self.stepx = ((random.randint(10,50))//10+levels)*random.choice([1,-1])
		self.stepy = (random.randint(10,50))//10+levels
...
def fire ():	
	...
	for e in enemies:
		e.timetick += 1 
		if e.type == 4:
			be = bullet_bye(e.x+e.size/2-5,e.y+e.size/2-5)
...
class Bullet():
	...
	#击中
	def hit(self):
		global score,scorea,bullet_timetick,golds,rage_mode,textf,boss_killed,levels,boss,level_enemies_now
		for e in enemies:
...
class Extra_bullet():
	...
	def hit(self):
		...
		for e in enemies:
			if(distance(self.x+5, self.y+5, e.x+e.size/2, e.y+e.size/2) < 30*e.size/64+bullet_hit_area) and not boss:
				shut_sound.play()
				scorea += (e.type**2)*300
				golds += (e.type**2)
				choose = random.randint(1,20)
...
while running:
	...
	if boss and not boss_killed and enemies == []:
		enemies.append(Enemy())
		level_enemies_now += 1
